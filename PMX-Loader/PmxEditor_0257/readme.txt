...
なお、Windows10であればディスプレイ設定からの全体拡大で、表示を崩すことなく対応が可能です。
詳細についてはダウンロードページなどをご確認ください。
...
[名称の一括編集] - 材質／ボーン／モーフ／表示枠／剛体／Joint／SoftBody の 名称(和名／英名)を一括で編集 ※各リストから呼び出し可能
...
[プラグインDLLの直接実行] - 外部のプラグインDLLを直接実行(ただし一度実行するとロード状態になるので、そのアセンブリDLLは更新不可になる ※Cプラグインの場合は破棄／リロードが可能)
...
[モデル表示] - モデル表示ウィンドウ(PmxView)を開く。非表示時はデータ変更時など Viewの同期更新が一部おこなわれなくなります。
...
※それぞれグラフィックボード(GPU)の機能に従います。デバイス側で固定されている場合などは変化ありませんのでご注意ください。
...
■鏡像固定対象 - 表示対象のみ
　鏡像固定(領域)対象を現在絞込み表示されているオブジェクトのみとします。
...
□操作系
■ドラッグ方向をハンドル操作と合わせる
　ドラッグ操作は [-]← / →[+] になるよう基本設定されていますが、有効時はハンドル操作と同一操作になるよう変更されます(標準ON)

■旧操作系(※非推奨機能)
　ドラッグ操作の回転が[Y][X][Z]になり、2段目のドラッグ方向が縦方向になります。
　法線などの操作系も多少変更になります。
※以前の操作系ですが仮想軸との相性が悪いので利用は推奨されません。

■頂点編集用に表示切替(※非推奨機能)
　表示を頂点編集用設定に切替
　なお表示設定での任意の設定と特に違いはありません。
...
[仮想軸] - 
 ...
 [視点向きを仮想軸方向に設定] - 画面の向きを仮想軸と一致するよう設定
...
○メニュー [編集]
[リストの更新] - 各チェックリストを更新します。リスト表示が不正になった場合などに。

[チェック要素の保存／復元] -  ※現在の表示対象リストのチェック状態をクリップボードへ保存／復元します。
...
[選択材質のパーツ削除] -
　材質に関連した一連のパーツデータを削除します(非常に広範囲に影響が出る場合があります)
...
■モーフ調整
　ナイフ切断時に既存の頂点／UVモーフが崩れないよう調整します(モーフ量が多い場合はそれなりの計算時間を有します)
...
[選択オブジェクト] -
 ...
 [選択範囲を隣接対象へ拡張] - 選択対象を隣接へ拡げます(詳細は下記参照)
 [選択範囲を縮小] - 選択対象を狭めます(詳細は下記参照)
...
この状態で PmxView側のモデル形状を変化させると、頂点モーフとして変形ができるようになります。変形状態はSubViewへ表示されます。
...
[編集] - 
 ...
 [自動的に相違選択絞込み(SubView)] - SubView側の表示頂点を自動的に相違選択で絞込みます(初期ON)

 [編集頂点の鏡像適用] - (移動)編集した頂点の鏡像側頂点に対して同様に鏡像移動を適用します(仕様制限を超えて無理やり実現している処理なので精度はあまりよくありません)
  [全ての編集済み頂点] - 編集対象を鏡像適用
  [選択頂点] - 選択対象のみ鏡像適用

 [頂点編集の初期化] - すべて初期状態へ戻します。
 [頂点編集の初期化-選択頂点のみ] - 選択対象を初期状態へ戻します。
 [選択頂点をモデル形状位置へ移動] - 選択対象をモデルの形状位置へ移動します。

 [材質モーフの新規作成] - 材質の変化状態に対して新規で材質モーフを作成します。
                          ※テクスチャモーフや全材質対象モーフは作成できません(モーフ編集から設定してください)
 [材質モーフの作成時の演算方式] - [0乗算+加算]/[加算] - モーフ作成時のパラメータ変化／0乗算+加算の場合は基準となる材質値を変更してもモーフ結果は同じになります。

 [モデルファイルから形状読み込み] - PMX/PMDファイルから形状をロードします。
 [モーフ編集開始時の形状を個別記憶へ送る] - モーフ編集開始時の形状(＝初期状態)を個別記憶として登録します。
※モーフ編集中にUndoなどを挟むと場合によって形状が初期状態へ戻らなくなるケースがあるかもしれません。形状を戻す必要に応じて利用してください。

 [オフセット値を現在のモーフ変形量に調整] - モーフ量≒スライダ設定値で現在の編集状態を倍率変化させます。
...
[編集状態選択枠の有効切り替え] - 編集状態選択枠の有効状態を切り替えます(一部の操作がうまく動作しないケースへの対応)
...
　[SDEF対応] にチェックを付けることでSDEFを含んだモデルに対してのスケール変形を正常化できます(同メニュー指定)
...
[ファイル] -
 [保存／更新時などの頂点モーフ正規化] - 以下の保存及び更新処理時における頂点モーフ正規化のON/OFF
...
※別の箇所で更新した場合は、全て自動的に更新するようになっていますが、場合によっては手動で更新する必要があるかもしれません。
 ...
 [ボーンの単一選択固定] - ボーン選択時の複数選択を抑制します。
 [ボーン選択情報を親Viewへ送る] - ボーンの選択状態を親View側へ送信します。TransformView → PmxView → Pmx編集 まで選択状態を同期化可能(ただし逆は不可)
 [モーフ選択情報を親Viewへ送る] - モーフの選択状態を親View側へ送信します。モーフ編集での既存モーフ選択に対応します。
...
[モード] -
 ...
 [モーションキーによるIK有効切り替え] - モーションデータからIKのON/OFFを随時切り替えます。同IK欄 [Key]

 [変形を1F進める] - 変形状態を更新し、フレーム位置を1F分進めます。
 [変形を1F戻す] - 変形状態を更新し、フレーム位置を1F分戻します。
